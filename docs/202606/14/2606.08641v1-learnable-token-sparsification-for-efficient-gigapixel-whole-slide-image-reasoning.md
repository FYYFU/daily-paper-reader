...
tldr: "全切片图像视觉语言模型面临token过多的问题，传统下采样或启发式剪枝方法会丢失分散的病理细节。本文提出可训练稀疏化框架，通过SparseLearn模块实现可微分token选择，推理时保留32个最高分token（仅0.78%）。在SlideBench上达到73.32%准确率，并零样本泛化到其他数据集。该方法解决了视觉上下文瓶颈，为端到端全切片图像推理提供了高效范式。"
source: arxiv
...
motivation: 现有token减少方法多为无训练的下采样或启发式剪枝，容易丢失分散在组织中的病理证据。
method: 提出可训练稀疏化架构，使用SparseLearn组件（含方差保持噪声门控和可微分Soft Top-K）实现梯度传播，推理时用硬Top-K保留32个token。
result: "在SlideBench(TCGA)上准确率73.32%，优于采样基线和通用VLM，并零样本泛化到其他数据集。"
conclusion: 解决了视觉上下文瓶颈，防止稀疏诊断证据被稀释，为全切片图像推理提供了高效范式。
---

## 摘要
视觉语言模型在处理十亿像素级全切片图像时面临一个主要困难，即视觉令牌数量过多。现有解决方案通常依赖于无需训练的空间下采样或启发式剪枝策略，而这些方法常常丢弃细微但具有临床意义的模式，因为病理证据在组织中不规则地分布。为克服这一限制，我们将全切片图像中的令牌减少问题重新表述为可训练的稀疏化问题，使模型能够学习最优选择策略，而非遵循固定启发式规则。我们提出了一种解耦路由架构。为在训练期间通过不可微的剪枝操作实现梯度传播，我们引入了一个名为SparseLearn的组件。该组件使用一个保持方差的噪声门，通过可微的Soft Top-K算子调节每个补丁的信息流，并结合一个对角注意力去噪器，在不泄漏空间信息的情况下恢复受扰动的表示。在推理时，SparseLearn模块被完全丢弃，训练好的评分器应用确定性Hard Top-K算子，仅保留得分最高的32个令牌，不产生额外计算。通过将视觉序列压缩至仅32个令牌的稀疏集（仅占原始长度的0.78%），我们的框架在SlideBench（TCGA）上实现了73.32%的整体准确率，持续优于基于采样的基线和通用视觉语言模型。它在SlideBench（BCNB）和WSI VQA*上也展现出强大的零样本泛化能力。通过解决视觉上下文瓶颈并防止稀疏诊断证据的稀释，这项工作为端到端十亿像素全切片图像推理提供了一种高效范式。
