...
## 速览
**TLDR**：现有安全基准多用单一规范提示词，但分数可能对表面形式敏感。本文以机器回译和代码切换生成五种等义改写，用统一评判器评估370个种子和5个模型。结果显示没有单一改写普遍更危险，但仅读规范形式会低估不安全合规：联合不安全率比最差单形式高3.3-12.9个百分点，5-13%规范安全样本在改写下变不安全。安全评测应覆盖多种表面形式，否则分数存在系统性盲区。 \
**Motivation**：现有安全基准以单一规范提示词打分，可能因表面形式变化而低估模型不安全行为，且缺少对改写多样性影响的系统评估。 \
**Method**：预先用机器回译和矩阵语言框架代码切换生成五种保持语义的改写，以统一人类锚定评判器打分，并验证意图保留，在370种子×5形式×5模型上评测。 \
**Result**：无单一改写普遍更危险，但规范形式低估不安全合规：联合不安全率超最差单形式3.3-12.9个百分点，5-13%规范安全样本在改写后变不安全。 \
**Conclusion**：安全基准应包含多种表面形式，规范分数存在系统性盲区；发布数据集和逐响应标签以支持进一步研究。
